...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //attiva gli shader (cubo principale)
        lightingS.use();
        ...
        glm::vec3 lightColor;
        lightColor.x = static_cast<float>(sin(glfwGetTime() * 2.0)); //calcolato un colore per la luce che cambia nel tempo
        lightColor.y = static_cast<float>(sin(glfwGetTime() * 0.7));
        lightColor.z = static_cast<float>(sin(glfwGetTime() * 1.3));
        //luce diffusa: dispersa dalla superficie dell'oggetto in tutte le direzioni
        glm::vec3 diffuseColor = lightColor * glm::vec3(0.5f); // riduce l'intensità della luce diffusa (moltiplicato per colore di prima)
        glm::vec3 ambientColor = diffuseColor * glm::vec3(0.2f); // riduce l'intesità della luce ambientale

        //setta il colore delle varie luci
        lightingS.setVec3("light.ambient", ambientColor);
        lightingS.setVec3("light.diffuse", diffuseColor);
        // luce speculare : rimbalza dalla superficie e crea riflessi bianco brillante
        lightingS.setVec3("light.specular", 1.0f, 1.0f, 1.0f);

        // proprietà del materiale

        //colore ambientale del materiale, aranzione. Rappresenta l'illuminazione base che l'oggetto riflette in modo uniforme da
        //tutte le direzioni
        lightingS.setVec3("material.ambient", 1.0f, 0.5f, 0.31f);
        //colore diffuso del materiale, arancione. Determina la riflessione diffusa della luce, dipende dall'angolo della luce
        //rispetto alla superficie
        lightingS.setVec3("material.diffuse", 1.0f, 0.5f, 0.31f);
        // colore speculare del materiale, grigio chiaro. Determina quanto l'oggetto riflette la luce in modo speculare, lucido
        lightingS.setVec3("material.specular", 0.5f, 0.5f, 0.5f); //non è troppo brillante
