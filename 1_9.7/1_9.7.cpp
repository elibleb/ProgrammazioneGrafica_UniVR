...
int main()
{
    ...
    Shader shader("1_9.7_shader.vs", "1_9.7_shader.fs");
    ...
    // VBO = vertex buffer object, contiene i dati dei vertici (posizione e texture)
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        glClearColor(0.678f, 0.847f, 1.0f, 1.0f); //imposto colore sfondo
        ...
        glm::mat4 model = glm::mat4(1.0f); // creo matrice identità, model gestisce la posizione, la rotazione e la scala dell'oggetto
        glm::mat4 view = glm::mat4(1.0f); //view simula la posizione della camera
        glm::mat4 projection = glm::mat4(1.0f); //projection definisce la proiezione 3D, come la scena è schiacciata su un piano 2D
        model = glm::rotate(model, glm::radians(-55.0f), glm::vec3(1.0f, 0.0f, 0.0f)); //matrice model, rappresenta l'oggetto 
        //ruotato di -55 gradi attorno all'asse x
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); //sposta la matrice view lungo l'asse z di -3.0f unità, è come 
        //muovere la telecamera indietro di 3 unità nello spazio 3D (l'oggetto si vede meglio)
        projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);//crea una matrice 
        //di proiezione prospettica, simula l'effetto di una fotocamera (oggetti più lontani appaiono più piccoli). FOV angolo di campo
        // visivo impostato a 45 gradi, viene definito il rapporto di aspetto e piani near e far (distanza massima e minima in cui gli
        // oggetti vengono randerizzati)
        // recupera le matrix uniform locations
        //restituisce l'indirizzo delle variabili uniform model e view nello shader
        unsigned int modelLoc = glGetUniformLocation(shader.ID, "model");
        unsigned int viewLoc = glGetUniformLocation(shader.ID, "view");
        // invia le matrici model e view ai rispettivi uniforms negli shader
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view[0][0]);
        // normalmente non cambia spesso, è definita fuori dal ciclo di render
