...
int main()
{
    ...
    Shader shader("1_8.17_shader.vs", "1_8.17_shader.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        glClearColor(1.0f, 0.8f, 0.6f, 1.0f); //imposta colore dello sfondo
        glClear(GL_COLOR_BUFFER_BIT); // funzione che utilizza lo stato

        // scale/rotate/transform
        glm::mat4 trans = glm::mat4(1.0f); //creazione matrice 4x4 di tipo glm::mat4 inizializzata come matrice identità, 
        //non modifica un oggetto quando viene applicata a esso
        trans = glm::rotate(trans, glm::radians(90.0f), glm::vec3(0.0, 0.0, 1.0)); //matrice trans aggiornata per applicare una
        //rotazione di 90 gradi attorno all'asse Z (indicata dal vettore glm::vec3(0.0, 0.0, 1.0)), senso antiorario (-90.0f orario)
        trans = glm::scale(trans, glm::vec3(0.5, 0.5, 0.5)); //matrice trans aggiornata per applicare una scalatura, 
        //glm::vec3(0.5, 0.5, 0.5) vettore che rappresenta la scala lungo gli assi X, Y e Z. L'oggetto viene ridotto della metà 
        //su tutti gli assi, scala di 0.5
        ...
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans)); //la matrice trans (che contiene la rotazione e la scalatura)
        //viene inviata al programma shader tramite l'uniform transform
        /*Parametri: 
        transformLoc: l'indirizzo dell'uniform transform
        1: numero di matrici da inviare
        GL_FALSE: flag che indica se la matrice deve essere trasposta prima di essere inviata
        glm::value_ptr(trans): restituisce un puntatore ai dati della matrice trans per inviarla tramite OpenGL.*/
