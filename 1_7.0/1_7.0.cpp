...
int main()
{
    ...
    Shader shader("1_7.0_shader.vs", "1_7.0_shader.fs");
    ...
    float vertices[] = {
        // posizione         // colore           // coordinate texture (2 float)
    ...
    glBindVertexArray(VAO); //associa un VAO a una variabile. Il VAO tiene traccia di tutti gli stati relativi ai 
                            //buffer di vertici (come VBO e altri attributi)
    glBindBuffer(GL_ARRAY_BUFFER, VBO); //lega il VBO al target GL_ARRAY_BUFFER per lavorare con i dati dei vertici
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);//carica i dati nel buffer VBO. Viene passato il tipo di buffer GL_ARRAY_BUFFER,
    //la quantità di dati in byte, calcolata con sizeof(vertices), i dati (vertices), e viene indicato come verranno utilizzati 
    // (GL_STATIC_DRAW indica che i dati non cambieranno spesso)
    ...
    //Imposta il filtro di minificazione, ossia definisce come OpenGL deve gestire la texture quando viene visualizzata con
