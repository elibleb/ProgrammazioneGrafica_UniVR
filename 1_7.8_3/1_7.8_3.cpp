...
int main()
{
    ...
    Shader shader("1_7.8_3_shader.vs", "1_7.8_3_shader.fs");
    ...
    float vertices[] = {
       ...
       //Le coordinate texture utilizzano una piccola porzione centrale della texture, un quadrato al centro della texture, 
       //come fonte di colore. Le coordinate non coprono tutta la texture ma solo una parte di essa, così facendo
       //rendering mostra solo una sezione un po' sfumata della texture
    };
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // Imposta il comportamento della texture quando le coordinate
        //S (orizzontali) eccedono i limiti. 
        //GL_CLAMP_TO_EDGE: quando le coordinate di texture escono dai limiti della texture, 
    ...
    // GL_NEAREST -> la texture non viene interpolata, verrà preso il colore del pixel più vicino alle coordinate di texture
    ...
    //Una volta che le texture sono state caricate possiamo liberare la memoria occupata dall'immagine originale
    stbi_image_free(data);

    // dice a opengl a quale textur unit appartiene ogni sampler 
    shader.use();
    // imposto manualmente lo shader prima di impostare l'uniform
    //La funzione glUniform1i assegna le texture agli uniform sampler nello shader
    //texture1 è associata al texture unit 0
    glUniform1i(glGetUniformLocation(shader.ID, "texture1"), 0);
    // oppure lo setto attraverso lo shader, texture2 è associata al texture unit 1.
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        glClearColor(0.7f, 0.9f, 0.9f, 1.0f); //funzione che imposta il colore
