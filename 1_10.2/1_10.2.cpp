...
int main()
{
    ...
    Shader shader("1_10.2_camera.vs", "1_10.2_camera.fs");
    ...
    // invia la matrice projection agli shader
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
    //crea una matrice di proiezione prospettica, FOV angolo di campo visivo (angolo di visualizzazione della scena) 
    // di 45 gradi,SCR_WIDTH/SCR_HEIGHT aiuta a mantenere correttamente le 
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float radius = 10.0f; //radius definisce la distanza della camera dal centro della scena, dove si trovano i cubi
        float camX = static_cast<float>(sin(glfwGetTime()) * radius); //camX e camZ calcolano la posizione della camera lungo un cerchio, 
        //utilizzando il tempo glfwGetTime() per creare un effetto di movimento circolare attorno all'origine
        float camZ = static_cast<float>(cos(glfwGetTime()) * radius);
        //glm::lookAt crea una matrice di vista che guarda dalla posizione della camera verso l'origine glm::vec3(0.0f, 0.0f, 0.0f),
        // con il vettore glm::vec3(0.0f, 1.0f, 0.0f) che definisce l'orientamento verso l'alto (asse Y)
