...
int main()
{
    ...
    //cubo principale
    Shader lightingS("2_15.1_lighting_maps.vs", "2_15.1_lighting_maps.fs");
    //cubo sorgente luminosa
    Shader lightCubeS("2_15.1_light_cube.vs", "2_15.1_light_cube.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        glClearColor(0.1f, 0.2f, 0.7f, 1.0f); //imposta il colore dello sfondo 
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // resetta il buffer di profondità (depth), funzione che utilizza lo stato

        //attiva shader (cubo principale)
        lightingS.use();
        lightingS.setVec3("light.position", lightPos);
        ...
        lightingS.setVec3("material.specular", 1.0f, 1.0f, 1.0f);
        lightingS.setFloat("material.shininess", 64.0f); //impsosta la lucentezza del materiale
        ...
        // esegue il bind della diffuse map
...
unsigned int loadTexture(char const* path)
{
    ...
    glGenTextures(1, &textureID); //genera una texture e restituisce un identificatore memorizzato in textureID
    ...
    if (data) //se l'immagine è stata caricata correttamente
    {
        ...
        glBindTexture(GL_TEXTURE_2D, textureID); //lega la tecture appena creata per lavoare su di essa
        //carica i dati dell'immagine nella texture
        glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, data);
        //Genera le mipmaps per la texture
        glGenerateMipmap(GL_TEXTURE_2D);

        // Imposta il comportamento della texture quando le coordinate
