...
//impostazione iniziale di percentage (20%)
...
int main()
{
    ...
    Shader shader("1_7.8_4_shader.vs", "1_7.8_4_shader.fs");
   ...
   // Anche colore e texture seguono lo stesso pattern
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        glClearColor(0.7f, 0.9f, 0.9f, 1.0f); //funzione che imposta colore sfondo
        ...
        // imposta il valore percentuale del mix
        //passa il valore percentage al fragment shader
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) //controlla quando viene premuto il tasto UP
    {
        percentage += 0.001f; //percentage aumenta di 0.001f (se non ha raggiunto 1.0f)
        if (percentage >= 1.0f)
            percentage = 1.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) //controlla quando viene premuto il tasto DOWN
    {
        percentage -= 0.001f; //percentage diminuisce di 0.001f (se non è già a 0.0f)
        if (percentage <= 0.0f)
            percentage = 0.0f;
    }
