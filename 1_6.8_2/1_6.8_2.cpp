...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // setta l'uniform
        float offset = 0.5f; //variabile di tipo float, inizializzata con 0.5f Questo valore rappresenta una distanza o
                             //"traslazione" usata nello shader (per spostare l'oggetto nello spazio)

        //invoca metodo setFloat della classe Shader, passa il nome della variabile uniform "offset" e il suo valore offset: 0.5
        shader.setFloat("offset", offset); //setFloat è una funzione che si occupa di inviare il valore della variabile offset 
        //al programma shader usando la funzione OpenGL glUniform1f. Il nome "offset" corrisponde al nome dell'uniform dichiarato
        //nello shader.
